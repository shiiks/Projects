			...
			<div class="main" >
				<form class="cbp-mc-form" enctype="multipart/form-data">
					<div class="cbp-mc-column">
	  					...
	  					<input type="text" id="full_name" ng-model="fullName" ng-pattern="/^[a-z A-Z]*$/" placeholder="Enter FullName"  title="Wrong Format"/>
	  					<label for="contact_no">Contact Number</label>
	  					<input type="text" id="conno" ng-model="contactNo" ng-pattern="/^[0-9]*$/" placeholder="Enter Contact Number"  title="Wrong Format"/>
	  					<label for="dob">Date Of Birth</label>
	  					<input type="date" id="dob" ng-model="dob" placeholder="Enter Date of Birth"/>
	  					<label for="location">Location</label>
	  					<input type="text" id="location" ng-model="location" ng-pattern="/^[a-z A-Z]*$/" placeholder="Enter Location" title="Wrong Format"/> 
	  				...
	  				<div class="cbp-mc-submit-wrap"><button class="cbp-mc-submit" ng-click="edit()">Edit</button></div>
...
userModule.controller('myappcontroller3', function ($scope,$http,$window) {
 ...
 $scope.id='<%
	    ...
	    out.print(userid);%>';
	      $http.get(urlBase + '/viewd/'+$scope.id).success(function(response) {
	    	  if(response==1){
	    		 
	    		  $window.location='${pageContext.request.contextPath}'+"/dashboard.jsp";
	    	  }
 ...
 $scope.edit = function edit() {
  if($scope.fullName==undefined || $scope.contactNo==undefined || $scope.dob == undefined || $scope.location == undefined || $scope.file == undefined){
   ...
   $http.post(urlBase + '/edit',dataObj).
