			...
			include "menu.php";
             ...
             <div class="body">
				<div id="welcome"> WELCOME <?php echo $data['name']; ?></div>
					...
					$desig=(isset($_POST['design']))?trim($_POST['design']):'';
					$mobile=(isset($_POST['emp_mobile']))?trim($_POST['emp_mobile']):'';
					$email=(isset($_POST['email']))?trim($_POST['email']):'';
					
					$query='insert into admin(admin_id,Name,sex,Designation,mobile,email,type) values ("'.mysql_real_escape_string($employee_id,$db).'",
                                          '.' "'.mysql_real_escape_string($emp_name,$db).'",
                                          '.' "'.mysql_real_escape_string($gender,$db).'",
                                           '.' "'.mysql_real_escape_string($desig,$db).'",
										   '.' "'.mysql_real_escape_string($mobile,$db).'",
										   '.' "'.mysql_real_escape_string($email,$db).'",
                                          '.' "'.mysql_real_escape_string("SA",$db).'")';
	          ...
	          $query1='insert into admin_login(admin_id,password) values ("'.mysql_real_escape_string($employee_id,$db).'",
					  ...
					  $result1=mysql_query($query1,$db) or die(mysql_error());
					   ...
					   if($result>0)
					     {
							 header("Location:add_admin.php?msg=5");
						  $msg=5;
						 }
					?>
