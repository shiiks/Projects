...
<html>
    <head>
            ...
            <title >HMS | Pending Request</title>
...
$query="SELECT i.Ticket_no,i.Roll,i.reporting_date,i.Indisciplinary_desc,r.hostel_id,s.branch,s.name FROM indisciplinary_dc_reports i,room_allocation1 r,student s WHERE i.Forward_status=0 AND i.student_id=r.student_id AND i.student_id=s.student_id AND r.end_date>NOW() AND i.Closure_status=0";
				...
				<div id="welcome"> WELCOME <?php echo $data['name']; ?></div>
                    ...
                    <div>
						...
						<tr>
							...
							<td><form method="POST" action="intermediate_auth.php"/>
							    <input id="actionyes" name="yes" type="submit" value="Yes"/>
								<input id="actionno" name="no" type="submit" value="No"/>
								</form>
							</td>
						</tr>
						<?php
...
if($_POST['yes'])
{
	$query4="UPDATE indisciplinary_dc_reports SET Forward_status=1,Intermediate_Auth='".$_SESSION['s_admin_username']."',Forward_Date=NOW() WHERE Ticket_no='".$ticket1['Ticket_no']."'";
...
{
	$query5="UPDATE indisciplinary_dc_reports SET Closure_status=1,Intermediate_Auth='".$_SESSION['s_admin_username']."' WHERE Ticket_no='".$ticket1['Ticket_no']."'";
